---
code: GVY7
name: "Vitality Profile (GVY7)"
nameZh: "全方位活力检查 (GVY7)"
category: screening
tagline: "Our flagship full-body, full-vitality screen."
taglineZh: "我们旗舰级的全身、全方位活力健康检查。"
description: "A robust, in-depth screening built for executives and anyone serious about their long-term health — covering metabolic, cardiac, antioxidant and vitamin status, plus 7 personalised add-ons."
descriptionZh: "为重视长期健康的行政人员及高需求人士打造的深入检查——涵盖代谢、心血管、抗氧化与维生素状态,另含任选 7 项个人化加配。"
price:
  promo: 556
  normal: 720
validUntil: 2026-06-30
features:
  - Full Blood Count
  - Iron Status (Iron, Transferrin, TIBC)
  - Renal Function Tests
  - Calcium
  - Phosphate
  - Magnesium
  - Liver Function Tests
  - Urine MicroAlbumin/Creatinine
  - Lipid Profile (Direct LDL if TG high)
  - Homocysteine
  - Apo-B & Apo-A
  - High-Sensitivity Troponin I with risk stratification
  - High-Sensitive CRP
  - Fasting Blood Sugar
  - HbA1c
  - Insulin & HOMA
  - Vitamin D
  - Vitamin B12
  - Folate
  - Superoxide Dismutase (SOD)
  - Urine FEME
addOn:
  selectCount: 7
  label: "Pick any 7 add-ons"
  items:
    - Estradiol (E2)
    - Progesterone
    - Testosterone
    - SHBG & FAI (when testosterone ordered)
    - LH
    - FSH
    - Prolactin
    - FOB (immuno method)
    - Cystatin C
    - Creatine Kinase
    - Amylase
    - Anti-Thyroid Peroxidase (ATPO)
    - Anti-Thyroglobulin Ab (ATAB)
    - Ferritin
    - PTH
    - TSH
    - Free T4 (FT4)
    - Free T3 (FT3)
    - HBs Ag & HBs Ab
    - HIV 1 & 2 (Ag & Ab)
    - Anti-HAV
    - H. pylori IgG
    - TP Ab (reflex RPR)
    - ABO & Rh
    - RFLD — RF & LDH
    - Anti-CCP
    - AFP
    - CEA
    - PSA
    - CA 125
    - CA 15-3
    - CA 19-9
    - EBV VCA IgA
    - Lipoprotein (a)
badge: "Most popular"
badgeZh: "最受欢迎"
order: 4
---
